...
(defn side-by-side-tables-offset-pdf
  "Single page with a nonzero MediaBox origin and two side-by-side ruled 5x3 tables. Returns byte[]."
  ^bytes []
  (with-open [doc (PDDocument.)]
    (let [page (PDPage. (PDRectangle. (float 100) (float 200)
                                      (float 841.8898) (float 585.2756)))]
      (.addPage doc page)
      (with-open [cs (PDPageContentStream. doc page)]
        (.setLineWidth cs (float 1.0))
